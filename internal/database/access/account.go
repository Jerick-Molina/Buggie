...
func InsertAccountByCode(usr *properties.User, companyCode string) int {
...
func QueryCredidentials(usr *types.User) int {

	sqlQuery := "select UserId from Users where Email = ? and Password = ?"
	if result := db.QueryRow(sqlQuery, usr.Email, usr.Password).Scan(&usr.Id); result != nil {
